...
public class NDD {
    ...
    private static NDD ANDRec(NDD a, NDD b) {
        ...
        if (a.field == b.field) {
            ...
            if (tempSet.size() == 1) {
                ...
                if (entry.getValue() == 1) {
                    // if (toBDD(entry.getKey()) != bdd.and(toBDD(a), toBDD(b))) {
                    // System.out.println("and");
                    // }
                    return entry.getKey();
                }
            }
            // if (toBDD(new NDD(a.field, tempSet)) != bdd.and(toBDD(a), toBDD(b))) {
            // System.out.println("and");
            // }
    ...
    private static NDD ORRec(NDD a, NDD b) {
        ...
        if (a.field == b.field) {
            ...
            while (itera.hasNext()) {
                ...
                while (iterb.hasNext()) {
                    Map.Entry<NDD, Integer> entryb = (Map.Entry<NDD, Integer>) iterb.next();
                    // if(test)System.out.println("tag1");
                    int intersect = bdd.ref(bdd.and(entrya.getValue(), entryb.getValue()));
                    // if(test)System.out.println("tag2");
                    if (intersect == 0)
                        continue;
                    int t = temp_a.get(entrya.getKey());
                    int n = bdd.ref(bdd.not(intersect));
                    // if(test)System.out.println("tag3");
                    temp_a.put(entrya.getKey(),
                            bdd.ref(bdd.and(t, n)));
                    // if(test)System.out.println("tag4");
                    bdd.deref(t);
                    t = temp_b.get(entryb.getKey());
                    // if(test)System.out.println("tag5");
                    temp_b.put(entryb.getKey(),
                            bdd.ref(bdd.and(t, n)));
                    // if(test)System.out.println("tag6");
                    bdd.deref(t);
                    bdd.deref(n);
            ...
            if (tempSet.size() == 1) {
                ...
                Map.Entry<NDD, Integer> entry = (Map.Entry<NDD, Integer>) iter.next();
                if (entry.getValue() == 1) {
                    // if (toBDD(entry.getKey()) != bdd.or(toBDD(a), toBDD(b))) {
            ...
            // if (toBDD(new NDD(a.field, tempSet)) != bdd.or(toBDD(a), toBDD(b))) {
            // System.out.println("or");
            // }
            return table.mk(a.field, tempSet);
        }
        else
        {
            ...
            while (itera.hasNext()) {
                ...
                // if(test)System.out.println("tag7");
                int temp = bdd.ref(bdd.and(false_set, n));
                // if(test)System.out.println("tag8");
                bdd.deref(false_set);
                bdd.deref(n);
                false_set = temp;
    ...
    public static NDD Diff(NDD a, NDD b) {
        NDD n = Not(b);
        NDD ret = AND(a, n);
        if(n != ret)table.testAndDelete(n);
        // System.out.println();
        ...
        if (a.field == field) {
            ...
            for (NDD next : a.edges.keySet()) {
                if (sum == null) {
                    ...
                    NDD old = sum;
                    sum = NDD.OR(sum, next);
                    if(old != sum)table.testAndDelete(old);
                }
            }
            return sum;
        } else {
